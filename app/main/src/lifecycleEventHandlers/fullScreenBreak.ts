...
export const setFullscreenBreakHandler = (
	...
	const { tray, trayTooltip, appWindow, contextMenu, fullscreenState } =
		appArgs;

	if (shouldFullscreen) {
		setFullScreen(true, alwaysOnTop, appWindow, fullscreenState);

		activateFullScreenShortcuts(() => {
			setFullScreen(false, alwaysOnTop, appWindow, fullscreenState);
		});
